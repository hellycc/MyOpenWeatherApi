...
fun <T> NetworkResult<T>.toFlow(): Flow<T> {
    ...
    return flow {
        when (response) {
            is NetworkResult.Success -> emit(response.value)
            is NetworkResult.Error -> throw CustomException(message = response.errorMessage)
            is NetworkResult.Exception -> throw CustomException(message = response.exception.message)
        }
    }
}
